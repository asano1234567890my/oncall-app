...
export function useScheduleApi({
  ...
  const fetchDoctors = async () => {
    try {
      ...
      console.error("ˆãŽtƒŠƒXƒg‚ÌŽæ“¾‚ÉŽ¸”s‚µ‚Ü‚µ‚½", err);
    }
  };
    ...
    try {
      ...
      const tasks = activeDoctors.map((doc) => {
        ...
        return fetch(`${apiUrl}/api/doctors/${doc.id}`, {
          ...
          const message = errData.detail || `ˆãŽtÝ’è‚Ì•Û‘¶‚ÉŽ¸”s‚µ‚Ü‚µ‚½: ${doc.name}`;
          throw new Error(message);
        });
      });

      await Promise.all(tasks);
      alert("‘Sˆõ‚Ì‹x‚ÝŠó–]EƒXƒRƒAÝ’è‚ð•Û‘¶‚µ‚Ü‚µ‚½");
      await fetchDoctors();
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "‘Sˆõ‚ÌÝ’è•Û‘¶‚ÉŽ¸”s‚µ‚Ü‚µ‚½";
      setError(message);
      alert(`Ý’è•Û‘¶‚ÉŽ¸”s‚µ‚Ü‚µ‚½: ${message}`);
    } finally {
  ...
  const handleDeleteMonthSchedule = async () => {
    if (!confirm(`${year}”N${month}ŒŽ‚ÌƒVƒtƒg‚ð‰æ–Êã‚ÅƒNƒŠƒA‚µ‚Ü‚·B‚æ‚ë‚µ‚¢‚Å‚·‚©H`)) return;
    ...
    try {
      ...
      setScores({});
      setSaveMessage("ŒÅ’è‚³‚ê‚Ä‚¢‚È‚¢ƒVƒtƒg˜g‚ðƒNƒŠƒA‚µ‚Ü‚µ‚½i¦‚Ü‚¾•Û‘¶‚³‚ê‚Ä‚¢‚Ü‚¹‚ñj");
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "‰æ–Êã‚ÌƒVƒtƒgƒNƒŠƒA‚ÉŽ¸”s‚µ‚Ü‚µ‚½");
    } finally {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (activeCount === 0) {
        throw new Error("—LŒø‚ÈˆãŽt‚ª‚¢‚È‚¢‚½‚ßAŽ©“®¶¬‚Å‚«‚Ü‚¹‚ñ");
      }
      ...
      const nonSunday = (day: number) => getWeekday(year, month, day) !== "“ú";
      ...
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        throw new Error(errData.detail || "Ž©“®¶¬‚ÉŽ¸”s‚µ‚Ü‚µ‚½");
      }
      ...
      setError(err instanceof Error ? err.message : "Ž©“®¶¬‚ÉŽ¸”s‚µ‚Ü‚µ‚½");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveToDB = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("•Û‘¶‚ÉŽ¸”s‚µ‚Ü‚µ‚½");
      }

      const data = await res.json();
      setSaveMessage(data.message);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "•Û‘¶‚ÉŽ¸”s‚µ‚Ü‚µ‚½");
    } finally {
      setIsSaving(false);
    }
  };
