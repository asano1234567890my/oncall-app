...
| 日付 | 内容 |
|------|------|
| 2026-03-26 | 共有入力ページ: 個別リンク（マジックリンク）に加えて、1つのURLで全医師がドロップダウンから名前を選んで不可日入力できる共通入力ページ（/entry/shared/[token]）を追加。カレンダーUIをDoctorEntryForm共通コンポーネントに抽出し個別/共有ページで共用。管理者の「まとめて共有」に共通ページURL/QRコード共有+[?]説明を追加。バックエンドはsystem_settings KVで共有トークン管理（DB変更なし） |
