...
- **短期（今月〜来月）**: P1-2 Phase2（複合制約診断）実装 + 自院での実運用開始
- **中期（2〜3ヶ月）**: Stripe決済 + 他院3〜5施設を開発パートナーとして巻き込む
- **長期（半年〜1年）**: 医師版をデファクトにする（有料30〜50施設・MRR 10〜25万円）
...
| 日付 | 内容 |
|------|------|
| 2026-03-25 | 細かい修正バッチ: ①AI解なし診断の冗長さ修正（閾値引き上げ: 日集中0.4→0.6・不可率0.30→0.60・曜日集中0.4→0.5、上位N件制限、Geminiプロンプト簡潔化200字/提案1-2件）②ソフト化ボタンをグレー控えめデザインに変更③PC不可日カレンダー修正（DashboardSettingsPanelを共通UnavailableDaysInputに置き換え）④ツールバー年月セレクタ小型化（text-xs・カスタム小三角矢印）⑤トークン画面Excelオフセット修正（B2起点）⑥ICSダウンロード修正（Content-Dispositionの日本語ファイル名をRFC5987 UTF-8エンコードに変更）⑦モバイル医師詳細に共有ドロップダウン追加（URL/LINE/メール/QR） |
