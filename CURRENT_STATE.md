...
**V2.1完了 → Phase1「売り物にする」進行中（P1-1/P1-2/P1-3/P1-5/P1-7/P1-7b/P1-9/P1-11/P1-12/P1-13/P1-16完了）**
<!-- P1-2: 事前チェック+MUS診断+管理者設定探索+人手不足検出+ソフト化+違反通知で全フロー完成（Geminiは一時スキップ・コード保持） -->
...
| P1-10 | ~~外部医師（ダミー医師方式）~~ → 外部枠方式で実装済み（Optimizer AddAtMostOne + 確定日カレンダー） | P1 | ✅ 完了 |
| P1-17 | 共有入力ページ（/entry/shared/[token]）— 1URLで全医師がドロップダウンから不可日入力 + DoctorEntryForm共通化 | P1 | ✅ 完了 |
| P1-18 | メアド必須化 — 新規登録時にメールアドレス必須。既存ユーザーにはログイン後にメアド入力促進バナー | P0 | 未着手 |
| P1-19 | 開発者管理ページ（/admin/dev）— アカウント一覧・利用状況・要望リスト・通知送信。is_superadminで制御 | P0 | 未着手 |
...
|--------|------|------|
| 無料 | ¥0 | 医師10人以下・基本機能 |
| スタンダード | 月3,000〜5,000円 | 人数無制限・AIガイド・カスタム制約 |
...
- **短期（今月〜来月）**: CORS修正+メアド必須化+開発者管理ページ → 自院実運用+知り合い5施設
...
| 日付 | 内容 |
|------|------|
| 2026-03-27 | 外部医師ダミー方式: 空欄方式→ダミー医師31人方式に全面刷新。DB is_external追加。オプティマイザAddExactlyOne統一。パレットに外部医師枠数±。日当直D&Dペア移動。外部医師のオレンジバッジ表示・全ハイライト。日当直モードのフロントエンド制約不整合修正（preventSunholConsecutive自動OFF・回数2倍カウント修正）。バリデーションメッセージクリア。カレンダー「なし」でexternal_fixed_datesクリア+日付バッジ月フィルタ。カレンダー月送りナビ表示修正 |
