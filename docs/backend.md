...
| `/api/schedule/export/{year}/{month}` | GET | `routers/schedule.py` | PDF/Excel出力（`?format=pdf\|xlsx`）。A4縦・2カラム・太め罫線。病院名+年月タイトル付き |
...
| `/api/schedule/ical/{doctor_token}` | GET | ICSフィード（Googleカレンダー同期用）— 医師個人の確定済みシフトを .ics 形式で返却。認証不要（トークン自体がアクセス権） |
...
| `settings_service.py` | システム設定のKVS Upsert（hospital_id スコープ）+ 仮保存スケジュール CRUD |
