...
| `/api/optimize/diagnose` | POST | `routers/optimize.py` | 解なし時の制約診断（3フェーズ: CP-SAT AddAssumptions→緩和再ソルブ→Gemini AI説明） |
...
| `/api/schedule/public/{doctor_token}/{year}/{month}` | GET | トークンで認証し公開月の全体スケジュールを医師名付きで返却（`{published, schedule, doctors}`）。認証不要 |
