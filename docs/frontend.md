...
| `/app` | 初心者向けメイン（生成+D&D+設定モーダル+ドロワー） | ✅ 完了 |
| `/dashboard` | **V2.1** D&D特化ダッシュボード（左=スケジュール全幅+右=医師パレット・設定スライドイン） | ✅ 完了 |
...
| `components/ScheduleBoard.tsx` | スケジュール表の表示とドラッグ&ドロップ編集（旧UI・`/app`のMobileScheduleBoard内で温存） |
| `components/MobileScheduleBoard.tsx` | モバイル専用スケジュール表（タップ操作・`/app`で使用・`core`プロップ1つで全機能） |
...
| `components/ScheduleCell.tsx` | スケジュール行の1セル（`ScheduleBoard`用） |
...
| `useScheduleDnd.ts` | ドラッグ&ドロップによるシフト編集ロジック（V2.1: `draggingDoctorId`・`cellValidityMap`追加） |
