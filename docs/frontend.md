...
| `/view` | `src/app/view/page.tsx` | **管理者用**当直表ページ（ヘルプツールチップ付き・公開/非公開トグル・編集・保存ドロップダウン（PNG/PDF/Excel）・月選択カスタムドロップダウン（公開=緑●/非公開=琥珀○）・未認証→/loginリダイレクト） | 必要 |
...
| `/entry/[token]` | `src/app/entry/[token]/page.tsx` | 個別リンク（医師が個別に不可日入力・固定不可曜日自己設定・確定シフト一覧（公開月のみ）・ICSカレンダーDL・不可日上限制御）。`DoctorEntryForm`共通コンポーネント使用 | 不要 |
| `/entry/shared/[token]` | `src/app/entry/shared/[token]/page.tsx` | 共通入力ページ（1つのURLで全医師が使える・ドロップダウンで名前選択→不可日入力）。`DoctorEntryForm`共通コンポーネント使用 | 不要 |
...
| `components/AiGuideChat.tsx` | AIガイドチャットウィジェット（右下フローティングボタン→スライドインパネル。PC: 400px右サイド / モバイル: 全画面。Claude API連携。/appと/dashboardに配置） |
| `components/BillingSection.tsx` | 課金管理セクション（プラン表示・Stripeアップグレード・Customer Portal。/appと/dashboardのアカウント設定に配置） |
...
| `DoctorManageDrawer.tsx` | 医師管理ドロワー（追加・編集・削除・個別ロック・共有ドロップダウン・一括共有・一括ロック/解除・ファイルから医師名一括取込・医師への案内メッセージ・個別不可日上限設定） |
...
| `useScheduleDnd.ts` | ドラッグ&ドロップによるシフト編集ロジック（モバイル用命令的API・ハイライト違反マップ）。**日当直ペア移動**: combinedモード祝日では通常時day+nightをペアでswap（day_shiftをnight_shiftに自動再同期）、強制モードでは個別swap可能 |
| `useScheduleConstraints.ts` | シフト配置の制約チェックロジック — `getPlacementConstraintMessage`（間隔・土曜上限・日祝上限・不可日等）・`getSwapConstraintMessage`。**外部医師（is_external=true）は全制約チェック対象外**。日当直モード時は`preventSunholConsecutive`自動OFF・day_shift検証スキップ |
...
| `useRealtimeScores.ts` | リアルタイムスコア計算（外部医師はscoreEntriesから除外、externalScoreTotal で合算返却） |
