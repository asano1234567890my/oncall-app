...
const CONTENT: Record<OnboardingSection, { title: string; lines: string[] }> = {
  ...
  "doctor-manage": {
    ...
    lines: [
      ...
      "「入力用URL」ボタンで医師専用リンクをコピーできます。このリンクをLINEやメールで送ると、医師が自分のスマホから休み希望を入力できます。",
      "入力された休み希望は自動でこちらに反映されるため、手動で転記する必要はありません。",
      "「ロック」をONにすると、その医師の入力を締め切れます。締切後の変更を防ぎたいときに使ってください。全員まとめてロック/解除も可能です。",
    ],
  },
  doctors: {
    title: "不可日設定について",
    lines: [
      ...
      "設定後は必ず「保存」を押してください。保存しないと変更は反映されません。",
    ],
  },
  "doctor-scores": {
    title: "スコア設定について",
  ...
  dnd: {
    title: "手動調整について",
    lines: [
      "入れ替えボタン（⇄）で2つの枠を選んでスワップできます。一覧（PC）画面ではドラッグ&ドロップでの入れ替えも可能です。",
      "鍵マーク（🔒）で枠をロックすると、再生成してもその枠は固定されます。「全🔒」で一括ロックも可能です。",
      "ロックした枠を残して「再作成」を押すと、ロックされていない枠だけがルールに沿って再生成されます。手動で一部を固定してから残りを最適化できます。",
