...
const CONTENT: Record<OnboardingSection, { title: string; lines: string[] }> = {
  ...
  "doctor-manage": {
    ...
    lines: [
      ...
      "「医師への案内メッセージ」を設定すると、各医師の入力画面の上部に表示されます。研究日の扱いや締切日などの連絡にご利用ください。",
      "「不可日の制限・解なし対策」では、医師1人あたりの不可日数の上限を設定できます。上限を超えて不可日を選べなくなります。",
      "スケジュール生成で解が見つからない場合は「全不可日をソフト化」で、個別不可日をなるべく尊重しつつも必要に応じて無視できるようにします。固定不可曜日はそのまま維持されます。",
    ],
  },
  doctors: {
    title: "不可日設定について",
